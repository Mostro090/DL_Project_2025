...
def tune_threshold_for_f1(y_true: np.ndarray, logits: np.ndarray) -> Tuple[float, BinaryMetrics]:
    probs = 1.0 / (1.0 + np.exp(-logits))
    # Ottimizzazione: se ci sono troppi punti, campioniamo
    candidates = np.unique(probs)
    if len(candidates) > 2000:
        candidates = np.percentile(candidates, np.linspace(0, 100, 2000))
...
def main():
    abs_model_dir = MODEL_DIR.resolve()
    print(f"Loading model from: {abs_model_dir}")
    ...
    # --- FASE 1: Tuning della soglia sul Validation Set ---
    final_thr = 0.5
    if TUNE_SPLIT in ds:
        print(f"\n--- TUNING SOGLIA SU: {TUNE_SPLIT} ---")
        dl_tune = make_dl(TUNE_SPLIT)
        y_val, logits_val = predict_task_a(model, dl_tune, device=DEVICE)
        
        if len(y_val) > 0:
            final_thr, best_m = tune_threshold_for_f1(y_val, logits_val)
            print(f"[TUNING] Soglia Ottimale trovata: {final_thr:.4f}")
            print(f"[TUNING] Best Val F1: {best_m.f1:.4f} (Prec: {best_m.precision:.2f}, Rec: {best_m.recall:.2f})")
        else:
            print(f"[WARNING] Validation set vuoto. Uso default 0.5")
    else:
        print(f"[WARNING] Split '{TUNE_SPLIT}' non trovato per il tuning. Uso default 0.5")

    # --- FASE 2: Evaluation sul Test Set usando la soglia trovata ---
    print(f"\n--- VALUTAZIONE SU {SPLIT} con Soglia {final_thr:.4f} ---")
    dl_eval = make_dl(SPLIT)
    y_true, logits = predict_task_a(model, dl_eval, device=DEVICE)
    
    if len(y_true) == 0:
        print(f"Nessun esempio valido trovato nello split '{SPLIT}'.")
        return
