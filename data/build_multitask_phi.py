...
def build_task_C_rows(rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    out = []
    for ex in rows:
        label = int(ex["label"])
...
def build_multitask_jsonl(
    ...
    train_out = build_task_A_rows(train_rows) + build_task_C_rows(train_rows)
    val_out = build_task_A_rows(val_rows) + build_task_C_rows(val_rows)
